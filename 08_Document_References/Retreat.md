...
- **UC28 (Housekeeping & Maintenance Management)**: Với tư cách là Quản lý Buồng phòng (Housekeeping Manager), tôi muốn xem danh sách các Villa đang ở trạng thái "Cần dọn dẹp" (DIRTY) để phân công dọn dẹp và cập nhật thành "Sẵn sàng" (AVAILABLE). Ngoài ra, nếu phát hiện hỏng hóc, tôi có thể chọn "Báo bảo trì", hệ thống sẽ yêu cầu nhập chi tiết lỗi (ví dụ: "Điều hòa hỏng") và lập tức đổi trạng thái Villa thành `OUT_OF_ORDER`. Trạng thái này sẽ khóa phòng, không cho Lễ tân gán khách mới cho đến khi kỹ thuật viên xác nhận đã sửa xong.
...
- **UC31 (Spa Inventory Auto-Tracking)**: Với tư cách là Quản lý/Hệ thống, tôi muốn hệ thống tự động trừ số lượng vật tư tiêu hao (tinh dầu, kem massage) trong kho Spa mỗi khi Chuyên viên Spa đánh dấu một phiên trị liệu là "Đã hoàn thành". Quản lý có thể xem Bảng điều khiển Kho và nhận cảnh báo (Low-stock alert) khi vật tư sắp hết.
- **UC32 (Loyalty & Tier Program)**: Với tư cách là Hệ thống, tôi muốn tự động cộng điểm "Aura Points" cho khách hàng sau khi Lễ tân hoàn tất thanh toán Hóa đơn gộp (UC22). Khách hàng sẽ được tự động thăng hạng (Silver, Gold) dựa trên điểm tích lũy và được áp dụng mã giảm giá khi đặt phòng lần sau. Quản lý có thể xem danh sách hạng thẻ khách hàng.
...
- **UC35 (Post-Retreat Wellness Assessment)**: Với tư cách là Khách hàng, tôi muốn thực hiện một bài đánh giá sức khỏe chuyên sâu (mức độ Stress, Giấc ngủ, Đau mỏi cơ bắp) trước và sau kỳ nghỉ. Hệ thống sẽ tự động tạo một biểu đồ đa giác (Radar Chart) phân tích sự cải thiện sức khỏe của tôi và hiển thị ở trang hồ sơ cá nhân.
- **UC36 (Incident & Service Recovery Management)**: Với tư cách là Quản lý, tôi muốn xem một "Bảng điều khiển Sự cố" (Incident Dashboard). Khi khách lưu trú đánh giá thấp trải nghiệm trong ngày hoặc phàn nàn trực tiếp, hệ thống tự động tạo một thẻ sự cố (Ticket). Tôi có thể theo dõi và phân công nhân viên xử lý sự cố này, đồng thời cập nhật trạng thái từ `OPEN` -> `IN_PROGRESS` -> `RESOLVED` để xoa dịu khách hàng ngay trong thời gian họ đang lưu trú.
