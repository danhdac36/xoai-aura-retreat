...
User -> Controller: [HTTP Method] /[path]\n[Request Body / Headers]
activate Controller
...
activate Repository
Repository -> PostgreSQL: [SQL/ORM operation]
Repository --> Service: [result]
deactivate Repository
Service -> Audit: emit([EventName])
Service --> Controller: [OutputDTO]
deactivate Service
Controller --> User: HTTP [StatusCode]\n[Response Body]
...
*(A placeholder for Error Path Sequence Diagram)*
...
ACTIVE --> ACTIVE : [Điều kiện - vd: re-consent]\n[Action]
note right of ACTIVE: Invariant: Không có chuyển tiếp nào\nđược XÓA record cũ (Append-only)
@enduml
```

> ⚠️ **Invariant bất biến:** Liệt kê các quy tắc trạng thái không bao giờ được vi phạm.

# 7. Domain Event Catalog

> ⭐️ **Section mới — EDS v2.0**
> Liệt kê tất cả domain events mà module này phát ra (publish) và tiêu thụ (consume). Đặt tên event theo quy tắc `[Entity][PastTenseVerb]` (vd: `ConsentGranted`).
...
```json
{
  "error": {
    ...
    "details": [{ "field": "fieldA", "message": "fieldA is required" }]
  }
}
```
