...
public class ItineraryServiceImpl implements ItineraryService {
        ...
        public ItineraryTimelineDTO getTimelineForGuest(Integer guestId) {
                User guest = userRepository.findById(guestId)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Không tìm thấy khách hàng với ID: " + guestId));
                ...
                if (activeBooking == null) {
                        throw new IllegalArgumentException("Khách hàng không có lịch trình hoạt động nào.");
                }
                ...
                List<Schedule> spaSchedules = scheduleRepository
                                .findByTreatmentBookingBookingIdAndIsDeleteFalseOrderByStartTimeAsc(
                                                activeBooking.getId());
                for (Schedule schedule : spaSchedules) {
                        ...
                        events.add(ItineraryTimelineDTO.TimelineEvent.builder()
                                        ...
                                        .location(schedule.getRoom() != null ? schedule.getRoom().getRoomName()
                                                        : "Aura Spa")
                ...
                for (MealOrder meal : mealOrders) {
                        events.add(ItineraryTimelineDTO.TimelineEvent.builder()
                                        ...
                                        .description("Bữa ăn theo Dietary Profile: "
                                                        + (meal.getNote() != null ? meal.getNote()
                                                                        : "Thanh lọc cơ thể"))
                                        .build());
                }
                        ...
                        if (reg.getSchedule() != null && !Boolean.TRUE.equals(reg.getSchedule().getIsDelete())) {
                                String instructorName = (reg.getSchedule().getInstructor() != null
                                                && reg.getSchedule().getInstructor().getUser() != null)
                                                                ? reg.getSchedule().getInstructor().getUser()
                                                                                .getFullName()
                                                                : "Huấn luyện viên";
                                events.add(ItineraryTimelineDTO.TimelineEvent.builder()
                                                .eventName("Yoga: " + reg.getSchedule().getYogaClass().getClassName())
                                                .time(reg.getSchedule().getStartTime())
                                                .location(reg.getSchedule().getLocation() != null
                                                                ? reg.getSchedule().getLocation()
                                                                : "Phòng tập Yoga")
                                                .description("Tham gia lớp học Yoga hướng dẫn bởi GV " + instructorName
                                                                + ". Thời lượng: "
                                                                + reg.getSchedule().getYogaClass().getDurationMinutes()
                                                                + " phút.")
                                                .build());
                        }
                }
                ...
                return ItineraryTimelineDTO.builder()
                                ...
                                .packageName(
                                                activeBooking.getRetreatPackage() != null
                                                                ? activeBooking.getRetreatPackage().getPackageName()
                                                                : "Chưa đăng ký gói")
                                .villaName(activeBooking.getAssignedVilla() != null
                                                ? activeBooking.getAssignedVilla().getVillaCode()
                                                : "Chưa xếp phòng")
                                .checkinDate(start)
                                .checkoutDate(end)
        ...
        @Override
        @org.springframework.transaction.annotation.Transactional(readOnly = true)
        public Page<com.AuraMoon.auramoon.booking.dto.BookingHistoryDTO> getBookingHistory(Integer guestId,
                ...
                return bookingsPage.map(b -> {
                        String packageName = b.getRetreatPackage() != null ? b.getRetreatPackage().getPackageName()
                                        : "Chua dang ky goi";
                        String villaName = b.getAssignedVilla() != null ? b.getAssignedVilla().getVillaCode()
                                        : "Chua xep phong";
                        return com.AuraMoon.auramoon.booking.dto.BookingHistoryDTO.builder()
                                        ...
                                        .totalAmount(b.getRetreatPackage() != null
                                                        && b.getRetreatPackage().getPrice() != null
                                                                        ? b.getRetreatPackage().getPrice().doubleValue()
                                                                        : 0.0)
