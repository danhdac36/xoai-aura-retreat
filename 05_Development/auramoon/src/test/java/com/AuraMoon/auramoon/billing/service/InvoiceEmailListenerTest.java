...
public class InvoiceEmailListenerTest {
    ...
    public void handleCheckoutEvent_NullEmail_StopsAndLogsMsg22() throws Exception {
        ...
        verify(auditLogRepository, times(1)).save(argThat(log -> 
            "MSG-22: No Email".equals(log.getDetails()) && log.getTargetId().equals(1001)
        ));
    }
    ...
    public void handleCheckoutEvent_SmtpError_CatchesExceptionAndLogsMsg23() throws Exception {
        ...
        doThrow(new RuntimeException("SMTP Failure")).when(emailNotificationService).sendInvoiceEmail("test@qa.com", dummyPdf);
        
        listener.handleCheckoutEvent(event);
        
        verify(auditLogRepository, times(1)).save(argThat(log -> 
            log.getDetails().contains("MSG-23") && log.getTargetId().equals(1001)
        ));
    }
}
